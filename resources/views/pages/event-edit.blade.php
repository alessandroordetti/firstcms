...
<section class="p-5">
    <div class="col-12">
        <h1 class="p-2">Modifica evento</h1>
    </div>

  <div class="container-fluid bg-white rounded border border-3 border-top py-4">
    <div class="row">
        <div class="col-10 mx-auto">
            <form action="{{route('event-update', $event->id)}}" method="POST" id="form">
                ...
                @elseif(session('response'))
                    <h2 class="text-center mb-3 text-success">{{session('response')}}</h2>
                @elseif(session('success'))
                    <h2 class="text-center mb-3 text-success">{{session('success')}}</h2>
                ...
                <div class="mb-3 d-flex align-items-center">
                    ...
                    <select class="form-select mb-3" aria-label="Default select example" name="ateneo">
                        ...
                        <option value="unipegaso" {{ ($event->ateneo == 'unipegaso') ? 'selected' : '' }}>Unipegaso</option>
                        <option value="mercatorum" {{ ($event->ateneo == 'mercatorum') ? 'selected' : '' }}>Mercatorum</option>
                    </select>
                </div>
                ...
                <div class="mb-3 d-flex">
                    ...
                    <div class="w-100 d-flex justify-content-between align-items-center rounded">
                        <input type="text" name="titolo" class="form-control" id="title" value="{{$event->titolo}}" required>
                ...
                <div class="mb-3 d-flex" id="slug-div">
                    ...
                    <input type="text" name="slug" class="form-control h-100" id="slug" value="{{$event->slug}}" required readonly placeholder="Per generare lo slug, inserisci nel campo titolo un valore valido dopodiché premi su Genera">            
                </div>

                <div class="mb-3 d-flex">
                    <label for="tipologia" class="mb-4 fw-bold me-2 w-25 text-center">Tipologia</label>
                    <input type="text" name="tipologia" class="form-control" id="tipologia" value="{{$event->tipologia}}" required>
                </div>
                
                <div class="mb-3 d-flex">
                    <label for="data" class="mb-4 fw-bold me-2 w-25 text-center">Data</label>
                    <input type="date" name="data" class="form-control" id="data" value="{{$event->data}}" required>
                </div>

                <div class="mb-3 d-flex">
                    <label for="time" class="mb-4 fw-bold me-2 w-25 text-center">Ora</label>
                    <input type="time" name="time" class="form-control" id="time" value="{{$event->ora}}" required>
                </div>

                <div class="mb-3 d-flex">
                    <label for="luogo" class="mb-4 fw-bold me-2 w-25 text-center">Luogo</label>
                    <input type="text" name="luogo" class="form-control" id="luogo" value="{{$event->luogo}}" required>
                </div>
                
                <div class="mb-4 d-flex justify-content-between">
                    <label for="contenuto" class="mb-4 fw-bold me-2 w-25 text-center">Contenuto</label>
                    <textarea id="summernote" name="contenuto">{{$event->contenuto}}</textarea>
                </div> 

                <div class="mb-3 d-flex">
                    <label for="ateneo" class="mb-4 fw-bold me-2 w-25 text-center">Titolo Seo</label>
                    <input type="text" name="seo_title" class="form-control" id="input-text" value="{{$event->seo_title}}" required>
                </div>

                <div class="mb-3 d-flex">
                    <label for="descrizione_seo" class="mb-4 fw-bold me-2 w-25 text-center">Descrizione Seo</label>
                    <input type="text" name="seo_description" class="form-control" id="input-text" value="{{$event->seo_description}}" required>
                </div>
                ...
                <div class="mb-3 d-flex ">
                    ...
                    <div class="text-start">
                        ...
                        <label class="switch">
                            ...
                            <input type="checkbox" name="stato" value="1" {{ ($event->stato == 1) ? 'checked' : ''}}>
